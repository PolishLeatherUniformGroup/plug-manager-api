...
export class MemberSuspendMembershipHandler implements ICommandHandler<MemberSuspendMembership> {
    constructor(
    ...
    async execute(command: MemberSuspendMembership) {
        try {
            ...
            member.suspendMember(command.suspendDate, command.suspendUntilDate, command.suspendReason, command.appealDeadline);
            member.commit();
        } catch (e) {
            console.error(e);
            throw e;
